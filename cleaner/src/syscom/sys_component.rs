...
pub fn trash_files_nsfilemanager(
    paths: &[PathBuf],
) -> Result<(Vec<(PathBuf, PathBuf)>, Vec<(PathBuf, ErrorKind)>)> {
    let mut failed_paths = Vec::new();
    let mut moved_paths = Vec::new();

    if paths.is_empty() {
        return Ok((moved_paths, failed_paths));
    }
    ...
    for (path, url) in paths.iter().zip(urls.iter()) {
        ...
        match fm.trashItemAtURL_resultingItemURL_error(url, out_url) {
            Ok(_) => {
                if let Some(result_url) = resulting_url.and_then(|url| url.path()) {
                    moved_paths.push((path.clone(), PathBuf::from(result_url.to_string())));
                } else {
                    failed_paths.push((
                        path.clone(),
                        ErrorKind::skipped()
                            .with_summary("Trash succeeded but no resulting path was returned")
                            .with_reason("NSURL::path() returned None"),
                    ));
                }
            }
            Err(error) => {
                ...
                let err = ErrorKind::failed()
                    .with_summary("Failed to move item to Trash")
                    .with_reason(reason);
